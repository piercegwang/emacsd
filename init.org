...
*** CANCELLED Open Link from =:LINK= Property                     :ARCHIVE:
CLOSED: [2019-12-27 Fri 16:07]
...
I later discovered that I didn't need this as =C-c C-o= already opens the first link under a headline if point is on the headline.
...
#+begin_src emacs-lisp :tangle no
  ...
  (define-key artist-mode-map (kbd "<f1>") 'artist-select-op-poly-line)
  (define-key artist-mode-map (kbd "<f2>") 'artist-select-op-pen-line)
  (define-key artist-mode-map (kbd "<f3>") 'artist-select-op-line)
  (define-key artist-mode-map (kbd "<f4>") 'artist-select-op-square)
  (define-key artist-mode-map (kbd "<f5>") 'artist-select-op-ellipse)
#+end_src
