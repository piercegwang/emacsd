...
(defun pgw/check-ohs-class (absolute classname semesters days period fallstart fallend springstart mononfri springend holidays)
...
is in the form of a list"
  (let ((dayname (calendar-day-of-week (calendar-gregorian-from-absolute absolute)))
        ...
        (time nil))
    (cond ((equal period 1) (setq time "06:00-07:15")) ;; Period times in PST/PDT
    ...
    (when (and (cond ((equal days '(1 3)) (or (memq dayname '(1 3)) (pgw/date-date absolute (nth 0 mononfri) (nth 1 mononfri) (nth 2 mononfri)))) ;; Account for MLK Monday on Friday
      ...
      (when (not (memq 't (mapcar (lambda (holiday) (if (> (length holiday) 3)
  ...
  ;; (mapcar (lambda (x) (if (> (length x) 3) (pgw/date-block absolute (nth 0 x) (nth 1 x) (nth 2 x) (nth 3 x) (nth 4 x) (nth 5 x)) (pgw/date-date absolute (nth 0 x) (nth 1 x) (nth 2 x)))) holidays)

(defun pgw/create-class (classname semesters days period)
...
3. The three integers represent M/W, T/Th, and F respectively"
  (let ((current (calendar-absolute-from-gregorian (diary-make-date 2020 8 19)))
    ...
    (while (pgw/date-block current (nth 0 fallstart) (nth 1 fallstart) (nth 2 fallstart)
      ...
      (let ((info (pgw/check-ohs-class current classname semesters days period fallstart fallend springstart mononfri springend holidays)))
        (when info
      ...
      #s(hash-table
         ...
         data (
               ...
               "holidays" ((2020 9 7) ;; Labor Day
                           (2020 9 11) ;; Back to School Night
                           (2020 10 28 2020 10 30) ;; Parent-Teacher Conferences (no classes)
